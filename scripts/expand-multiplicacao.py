...
conn = mysql.connector.connect(
    ...
    database=os.environ.get('DB_NAME', 'arquimedes')
)
cursor = conn.cursor()

# Conteúdo expandido para Página 1 de Multiplicação (parte 1)
content_page1_part1 = """# Multiplicação: Soma Repetida
...
# Atualizar Página 1
cursor.execute(
    "UPDATE pages SET mainText = %s WHERE slug = 'soma-repetida'",
    (content_page1_part1,)
)

print(f"✅ Página 1 de Multiplicação atualizada ({len(content_page1_part1)} caracteres)")

conn.commit()
cursor.close()
conn.close()

print("✅ Todas as páginas de Multiplicação foram expandidas com sucesso!")
